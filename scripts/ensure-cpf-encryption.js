...
const OLD_INDEX_NAME = "uk_usuarios_instituicao_cpf";
const NEW_INDEX_NAME = "uk_usuarios_instituicao_cpf_hash";
...
async function main() {
  ...
  try {
    ...
    if (!(await indexExists(connection, NEW_INDEX_NAME))) {
      await connection.query(
        `ALTER TABLE ${TABLE_NAME} ADD UNIQUE KEY ${NEW_INDEX_NAME} (instituicao_id, cpf_hash)`,
      );
      console.log(`Índice ${NEW_INDEX_NAME} criado.`);
    }

    console.log("Migração de criptografia de CPF concluída.");
  } finally {
